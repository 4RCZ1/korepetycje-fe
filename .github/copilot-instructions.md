...
- State & side-effects: most data flows are encapsulated in `hooks/*` (e.g. `useAuth.ts`, `useResourceApi.ts`, `useScheduleApi.ts`).
- UI: reusable UI lives under `components/` (theme-aware components prefixed `Themed*`, and small feature folders like `ResourceGroups`, `Schedule`).
...
- **ResourceGroup** (`ResourceGroupType`): named collection of resources. Simplifies bulk assignments.
...
Mocks & testing
- Some endpoints have mock implementations under `services/mock/` (e.g. `resourceGroupApi.ts`) for local development when backend is unavailable.
...
- Routing: modify files inside `app/` to add pages/screens. Keep nested routes under folders like `(tabs)`, `(auth)`.
- Theming: use `useThemeColor` and the `Themed*` components for colors and text instead of raw color strings.
...
- Storage: helpers in `utils/storage.ts` (AsyncStorage wrappers). Use storages keys consistent with existing code (`auth_token`, `auth_user`).
...
- Themed UI & color utilities: `components/ThemedText.tsx`, `components/ThemedView.tsx`, `hooks/useThemeColor.ts`, `constants/Colors.ts`, `utils/colors.ts`
- Routes & UI structure: `app/` directory (file-based routing). Example pages: `app/(tabs)/schedule.tsx`, `app/(auth)/login.tsx`.

Editing guidelines for agents
- Keep changes small and focused: update only files relevant to the feature or bug.
- Maintain TypeScript types — use types from `types/` when available (e.g. `types/resource.ts`).
